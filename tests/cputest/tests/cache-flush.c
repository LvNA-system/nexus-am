  ...
  for (int j = 0; j < 1; j++) { \
    for (s = 0; s < sizeof(sizes)/sizeof(int); s++) { \
      ...
      for (unsigned int i = 0; i < steps; i++) { \
        int idx = ((fast_rand(&g_seed) & mask) | (mhartid << 1)) & lengthMod; \
...
int main() {
  ...
  int sizes[] = { 4 * MB };
  // assume at most 8cores, cache line size = 64byte, that is 16 uint32
  // different core access different part of cache line
  int s;
  int k = 0;
  while (1) {
    k++;
